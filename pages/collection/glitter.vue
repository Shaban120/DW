            ...
            <v-row>
                <v-col md="6" cols="12">
                    <div class="Collection_Header">
                        <h1 class="belgin-bold">Glitter</h1>
                    </div>
                </v-col>
                <v-col md="6" cols="12">
                    <div class="Collection_Header">
                        <p class="poppins-regular">Want an interior with disco inspirations? Sparkling originality? Count on our Glitter range and its glam & glossy and unique looks! Whether they are colorful or not, your pieces will shine brightly once combined with optimal lighting.</p>
                    </div>
                </v-col>
            </v-row>
            <v-row>
...
<script>
import glitterData from "~/Public/glitterData.json";
import CollectionComponent from "../../components/CollectionComponent.vue";

export default {
    head() {
    return {
      title: 'Distinction Wrapping Glitter Floor Tiles',
      meta: [
        {
          hid: 'description',
          name: 'description',
          content: "Want your home to sparkle? Distinction Wrapping's Glitter floor tiles come in many colors and shine brightly when the light hits them. Make a statement everyone will remember!"
        }
      ]
    }
  },
    ...
    computed: {
        ...
        categories() {
            const uniqueCategories = new Set(this.dataList.props.pageProps.products.glitterData.map(item => item.attributes.product_category.data.attributes.Name));
            return Array.from(uniqueCategories);
        },
        filteredItems() {
            let filteredItems = this.dataList.props.pageProps.products.glitterData;
    ...
    methods: {
        ...
        navigateToPage(item) {
            this.$router.push({ path: `/collection/glitter/${item.attributes.slug}` });
        },
        getColWidth(index) {
    ...
    mounted() {
        this.totalItems = this.dataList.props.pageProps.products.glitterData.length;
    },
    watch: {
        filteredItems() {
