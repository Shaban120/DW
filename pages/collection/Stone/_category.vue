...
<script>
import jsonData from "~/Public/stoneData.json";
...
this.$router.push({ path: `/cart/${item.attributes.slug}` })
        ...
        getObjectByCategory(category) {
            const stoneData = jsonData.props.pageProps.products.stoneData;
            this.Products = stoneData.filter(obj => obj.attributes.slug === category);
