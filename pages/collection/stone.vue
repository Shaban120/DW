            ...
            <v-row>
                <v-col md="6" cols="12">
                    <div class="Collection_Header">
                        <h1 class="belgin-bold">Stone</h1>
                    </div>
                </v-col>
                <v-col md="6" cols="12">
                    <div class="Collection_Header">
                        <p class="poppins-regular">From the floor to the walls, make multiple shades of colors your own to dress up every room. The charm of a 'natural stone' appearance in a room? The impression of small, controlled imperfections... </p>
                    </div>
                </v-col>
            </v-row>
            <v-row>
...
<script>
import stoneData from "~/Public/stoneData.json";
import CollectionComponent from "../../components/CollectionComponent.vue";

export default {
    head() {
    return {
      title: 'Natural Stone Look Floor Tiles for Every Room',
      meta: [
        {
          hid: 'description',
          name: 'description',
          content: "Discover the charm of natural stone-look floor tiles from Distinction Wrapping. Elevate your space with multiple shades and controlled imperfections for a unique touch."
        }
      ]
    }
  },
    ...
    computed: {
        ...
        categories() {
            const uniqueCategories = new Set(this.dataList.props.pageProps.products.stoneData.map(item => item.attributes.product_category.data.attributes.Name));
            return Array.from(uniqueCategories);
        },
        filteredItems() {
            let filteredItems = this.dataList.props.pageProps.products.stoneData;
    ...
    methods: {
        ...
        navigateToPage(item) {
            this.$router.push({ path: `/collection/stone/${item.attributes.slug}` });
        },
        getColWidth(index) {
    ...
    mounted() {
        this.totalItems = this.dataList.props.pageProps.products.stoneData.length;
    },
    watch: {
        filteredItems() {
