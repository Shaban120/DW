            ...
            <v-row>
                <v-col md="6" cols="12">
                    <div class="Collection_Header">
                        <h1 class="belgin-bold">Textile</h1>
                    </div>
                </v-col>
                <v-col md="6" cols="12">
                    <div class="Collection_Header">
                        <p class="poppins-regular">Ideal for a modern and deconstructed look! Attract the eye and evoke emotion in a bedroom or dining room with the hypnotic textures of the many models in the range.</p>
                    </div>
                </v-col>
            </v-row>
            <v-row>
...
<script>
import textileData from "~/Public/textileData.json";
import CollectionComponent from "../../components/CollectionComponent.vue";

export default {
    head() {
    return {
      title: 'Distinction Wrapping Fabric Floor Tiles',
      meta: [
        {
          hid: 'description',
          name: 'description',
          content: "Make your home feel cozy and interesting with Distinction Wrapping's Textile floor tiles. We have many different styles to add a special touch to your floors."
        }
      ]
    }
  },
    ...
    computed: {
        ...
        categories() {
            const uniqueCategories = new Set(this.dataList.props.pageProps.products.textileData.map(item => item.attributes.product_category.data.attributes.Name));
            return Array.from(uniqueCategories);
        },
        filteredItems() {
            let filteredItems = this.dataList.props.pageProps.products.textileData;
    ...
    methods: {
        ...
        navigateToPage(item) {
            this.$router.push({ path: `/collection/textile/${item.attributes.slug}` });
        },
        getColWidth(index) {
    ...
    mounted() {
        this.totalItems = this.dataList.props.pageProps.products.textileData.length;
    },
    watch: {
        filteredItems() {
