...
<script>
import jsonData from "~/Public/colorData.json";
...
this.$router.push({ path: `/cart/${item.attributes.slug}` })
        ...
        getObjectByCategory(category) {
            const colorData = jsonData.props.pageProps.products.colorData;
            this.Products = colorData.filter(obj => obj.attributes.slug === category);
