            ...
            <v-row>
                <v-col md="6" cols="12">
                    <div class="Collection_Header">
                        <h1 class="belgin-bold">Concrete</h1>
                    </div>
                </v-col>
                <v-col md="6" cols="12">
                    <div class="Collection_Header">
                        <p class="poppins-regular">Discover the beauty of concrete's raw elegance without the weight and hassle – it's the ideal choice for those who crave an urban edge in their surroundings</p>
                    </div>
                </v-col>
            </v-row>
            <v-row>
...
<script>
import concreteData from "~/Public/concreteData.json";
import CollectionComponent from "../../components/CollectionComponent.vue";

export default {
    head() {
    return {
      title: 'Urban Elegance with Concrete-Inspired Walls',
      meta: [
        {
          hid: 'description',
          name: 'description',
          content: "Elevate your space with Distinction Wrapping's concrete walls. Experience the raw elegance of concrete without the weight and hassle."
        }
      ]
    }
  },
    ...
    computed: {
        ...
        categories() {
            const uniqueCategories = new Set(this.dataList.props.pageProps.products.concreteData.map(item => item.attributes.product_category.data.attributes.Name));
            return Array.from(uniqueCategories);
        },
        filteredItems() {
            let filteredItems = this.dataList.props.pageProps.products.concreteData;
    ...
    methods: {
        ...
        navigateToPage(item) {
            this.$router.push({ path: `/collection/concrete/${item.attributes.slug}` });
        },
        getColWidth(index) {
    ...
    mounted() {
        this.totalItems = this.dataList.props.pageProps.products.concreteData.length;
    },
    watch: {
        filteredItems() {
