            ...
            <v-row>
                <v-col md="6" cols="12">
                    <div class="Collection_Header">
                        <h1 class="belgin-bold">Wood</h1>
                    </div>
                </v-col>
                <v-col md="6" cols="12">
                    <div class="Collection_Header">
                        <p class="poppins-regular">The wood imitation adhesive allows you to bring a touch of nature
                            into your space. Strong and easy to install, it is the perfect product to change the design
                            of a room in the blink of an eye.</p>
                    </div>
                </v-col>
            </v-row>
            <v-row>
...
<script>
import woodData from "~/Public/woodData.json";
import CollectionComponent from "../../components/CollectionComponent.vue";

export default {
    head() {
    return {
      title: 'Wood Adhesive Sheets for Natural Interior Design',
      meta: [
        {
          hid: 'description',
          name: 'description',
          content: "Enhance your space with Distinction Wrapping's wood adhesive sheets. Transform your interior effortlessly with durable and easy-to-install plywood sheets."
        }
      ]
    }
  },
    ...
    computed: {
        ...
        categories() {
            const uniqueCategories = new Set(this.dataList.props.pageProps.products.woodData.map(item => item.attributes.product_category.data.attributes.Name));
            return Array.from(uniqueCategories);
        },
        filteredItems() {
            let filteredItems = this.dataList.props.pageProps.products.woodData;
    ...
    methods: {
        ...
        navigateToPage(item) {
            this.$router.push({ path: `/collection/wood/${item.attributes.slug}` });
        },
        getColWidth(index) {
    ...
    mounted() {
        this.totalItems = this.dataList.props.pageProps.products.woodData.length;
    },
    watch: {
        filteredItems() {
