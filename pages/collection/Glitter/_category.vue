...
<script>
import jsonData from "~/Public/glitterData.json";
...
this.$router.push({ path: `/cart/${item.attributes.slug}` })
        ...
        getObjectByCategory(category) {
            const glitterData = jsonData.props.pageProps.products.glitterData;
            this.Products = glitterData.filter(obj => obj.attributes.slug === category);
